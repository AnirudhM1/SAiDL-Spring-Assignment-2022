...
from dqn.model_barlow import DQN
...
class DQNAgent:
    def __init__(self,
        ...
        # self.momentum_encoder = MomentumEncoder(observation_space).to(device)s
    ...
    def optimise_td_loss(self):
        ...
        input_q_values, embeddings = self.policy_network(states)
        self.policy_network.calculate_representation_loss=False
        input_q_values = input_q_values.gather(1, actions.unsqueeze(1)).squeeze()

        loss = F.smooth_l1_loss(input_q_values, target_q_values) + self.representation_loss(embeddings)
    ...
    def representation_loss(self, embeddings):
        off_diagonal_weight = int(5e-3)
        z_a, z_b = embeddings
        N, D = z_a.shape

        z_a_norm = (z_a-torch.mean(z_a, 0))/torch.std(z_a, 0)
        z_b_norm = (z_b-torch.mean(z_b, 0))/torch.std(z_b, 0)

        # print(z_a_norm.shape)
        # print(z_b_norm.shape)

        c = torch.mm(z_a_norm.T, z_b_norm) / N

        diagonal = c_diff = torch.pow((c - torch.eye(D, device=self.device)), 2)
        c_diff.diagonal(dim1=-1, dim2=-2).zero_() # Setting all diagonal elements to 0

        loss = diagonal.sum() + off_diagonal_weight*c_diff.sum()
